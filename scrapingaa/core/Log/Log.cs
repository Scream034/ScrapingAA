...
public static class Log
{
	...
	private static void ProcessMessagePrint(in LogMessage message)
	{
		...
		{
			case LogLevel.Info:
				GD.Print(message.Value);
				break;

			case LogLevel.Warning:
				GD.PushWarning(message.Value);
				break;

			case LogLevel.Error:
				GD.PushError(message.Value);
				break;
		}
	}
	...
	private static void LoggingThreadWorker()
	{
		while (_loggingEnabled)
		{
			if (_logQueue.TryDequeue(out var message))
			{
				if (message is null)
				{
					GD.PushWarning("Log Thread Error: Dequeued null message");
					continue;
				};
				...
				catch (Exception ex)
				{
					...
					GD.PushError($"Log Thread Error: Failed to write to log file: {ex.Message}");
				}
			}
			else
			{
